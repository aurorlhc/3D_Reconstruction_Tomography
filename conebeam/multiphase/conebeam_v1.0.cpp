//reconstructing a three phase materials in 3D
...
#define MAXY 100
#define MAXZ 100
#define Lmax 150 //determing the receptor resolution, the unit is one pixel
...
#define NP 1000000 //number of all the particles
...
//the difference between temp_projection and projection
...
int counter_1 = 0;//count how many pixels have phase = 1
int counter_2 = 0;//count how many pixels have phase = 2 
...
void read_config()
{
  ...
  //now reduce the phase number from 5 to 3. combine the three phases with bigger numbers
	  ...
	  for(int l=0; l<MAXZ; l++)
	    {
	      ...
	      temp_bin = (n - y_c)*cos(i*d_theta) - (m - x_c)*sin(i*d_theta);

	      temp_j = l - z_c;

	      temp_pro_x = (int)floor(fanprojection(temp_l, temp_bin) + Lmax);
	      temp_pro_y = (int)floor(fanprojection(temp_j, temp_bin) + Lmax);
		...
		{
		  if(temp_pro_x<(2*Lmax+1) && temp_pro_x>=0 && temp_pro_y<(2*Lmax+1) && temp_pro_y>=0)
		    projection[i][temp_pro_x][temp_pro_y] += mu_3;
		}
...
int get_point_projection_x(int temp_x, int temp_y, int temp_z, double temp_theta)
{
  ...
  temp_bin = (temp_y - y_c)*cos(temp_theta) - (temp_x - x_c)*sin(temp_theta);
...
int get_point_projection_y(int temp_x, int temp_y, int temp_z, double temp_theta)
{
  ...
  temp_pro_y = (int)floor(fanprojection(temp_i, temp_bin) + Lmax);
 
  return temp_pro_y;
}

//initilaize the configuration from the total number of phase one and phase two pixels
...
void init_reconstr()
{
  ...
  for(int i=0; i<counter_2; i++)
	...
	while(config[m][n][l] == 2 || config[m][n][l] == 1)
	  {
	    m = rand()%MAXX;
	    n = rand()%MAXY;
	    l = rand()%MAXZ;
	...
	for(int n=0; n<MAXY; n++)
	  for(int l=0; l<MAXZ; l++)
	  {
		...
		if(temp_pro_x<(2*Lmax+1) && temp_pro_x>=0 && temp_pro_y<(2*Lmax+1) && temp_pro_y>=0)
	      ...
	      {
		if(temp_pro_x<(2*Lmax+1) && temp_pro_x>=0 && temp_pro_y<(2*Lmax+1) && temp_pro_y>=0)
		  temp_projection[i][temp_pro_x][temp_pro_y] += mu_3;
	      }	     
	  }
...
void get_projection_difference()
{
  ...
  if(config[indexi][indexj][indexk] != config[indexm][indexn][indexl])
      ...
      for(int i=0; i<N_p; i++)
	{
	  ...
	  else if(config[indexi][indexj][indexk] == 2 && config[indexm][indexn][indexl] == 1)
	    {
	      changed_projection[i][0] = mu_1 - mu_2;
	      changed_projection[i][1] = mu_2 - mu_1;
	    }
	  else if(config[indexi][indexj][indexk] == 1 && config[indexm][indexn][indexl] == 2)
	    {
	      changed_projection[i][0] = mu_2 - mu_1;
	      changed_projection[i][1] = mu_1 - mu_2;
	    }
	  else if(config[indexi][indexj][indexk] == 1 && config[indexm][indexn][indexl] == 0)
	    {
	      changed_projection[i][0] = mu_2 - mu_3;
	      changed_projection[i][1] = mu_3 - mu_2;
	    }
	  else if(config[indexi][indexj][indexk] == 0 && config[indexm][indexn][indexl] == 1)
	    {
	      changed_projection[i][0] = mu_3 - mu_2;
	      changed_projection[i][1] = mu_2 - mu_3;
	    }
	  else if(config[indexi][indexj][indexk] == 2 && config[indexm][indexn][indexl] == 0)
	    {
	      changed_projection[i][0] = mu_1 - mu_3;
	      changed_projection[i][1] = mu_3 - mu_1;
	    }
	  else if(config[indexi][indexj][indexk] == 0 && config[indexm][indexn][indexl] == 2)
	    {
...
double get_dE()
{
	...
	for(int i=0; i<N_p; i++)
	  dE += (2*diff_projection[i][changed_location[i][0][0]][changed_location[i][0][1]]*changed_projection[i][0] - changed_projection[i][0]*changed_projection[i][0]) + (2*diff_projection[i][changed_location[i][1][0]][changed_location[i][1][1]]*changed_projection[i][1] - changed_projection[i][1]*changed_projection[i][1]);              
...
void print_final_projections()
{
  ...
  for(int i=0; i<N_p; i++)
	...
	for(int p=0; p<(2*Lmax+1); p++)
	{
	  fout2<<(r-Lmax)<<" "<<(p-Lmax)<<" "<<temp_projection[i][r][p]<<endl;
	}
