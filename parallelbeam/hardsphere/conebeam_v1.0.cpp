...
void init_reconstr()
{
  ...
  ofstream fout;
  fout.open("initial_diff_projections.txt");
  for(int i=0; i<N_p; i++)
    for(int j=0; j<(2*Lmax+1); j++)
      for(int k=0; k<(2*Lmax+1); k++)
	{
	  fout<<i<<" "<<j<<" "<<k<<" "<<diff_projection[i][j][k]<<" "<<diff_projection[i][j][k]*diff_projection[i][j][k]<<endl;
	}
  fout.close();
}
...
void get_projection_difference()
{
  ...
  if(config[indexi][indexj][indexk] != config[indexm][indexn][indexl])
      ...
      for(int i=0; i<N_p; i++)
	{
	  ...
	  else if(config[indexi][indexj][indexk])
	    {
...
double get_dE()
{
	...
	for(int i=0; i<N_p; i++)
	  {
	    if(changed_location[i][0][0]<(2*Lmax+1) && changed_location[i][0][0]>=0 && changed_location[i][0][1]<(2*Lmax+1) && changed_location[i][0][1]>=0)
	    ...
	    if(changed_location[i][1][0]<(2*Lmax+1) && changed_location[i][1][0]>=0 && changed_location[i][1][1]<(2*Lmax+1) && changed_location[i][1][1]>=0)
...
void print_final_diff_projections()
{
  ...
  fout3.open("final_diff_projections.txt");
