...
#define N_p 20 //number of projections, which are assumed to be evenly distributed over pi  
...
double projection[N_p+1][2*Lmax+1][2*Lmax+1]; //the set of projections. 
                                            //fan beam projection length equal to (2*Lmax+1) for test....

double temp_projection[N_p+1][2*Lmax+1][2*Lmax+1];
...
double diff_projection[N_p+1][2*Lmax+1][2*Lmax+1];
...
int changed_location[N_p+1][2][2]; //the projected location of the changed pixel
...
{
  double temp_dist1 = (temp_x - x_c)*(temp_x - x_c) + (temp_y - y_c)*(temp_y - y_c);
  
  return sqrt(temp_dist1);
}
...
{
  double val_1 = (double)(temp_y - y_c)/(double)(temp_x - x_c);

  double temp_theta = atan(val_1);
...
void get_projection_difference()
{
  ...
  if(config[indexi][indexj][indexk] != config[indexm][indexn][indexl])
      ...
      for(int i=0; i<N_p+1; i++)
	{
	  ...
	  if(config[indexi][indexj][indexk] == 1)
	    {
  ...
  for(int i=0; i<N_p; i++)
    {
      for(int j=0; j<(2*Lmax+1); j++)
	for(int k=0; k<(2*Lmax+1); k++)
	sum += (projection[i][j][k] - temp_projection[i][j][k]) * (projection[i][j][k] - temp_projection[i][j][k]);
    }

  return sum; //sum/(N_p*(2*Lmax+1)*(2*Lmax+1));
}
*/
...
double get_dE()
{
	...
	for(int i=0; i<N_p+1; i++)
	  {
	    dE_1 += (2*diff_projection[i][changed_location[i][0][0]][changed_location[i][0][1]]*changed_projection[i][0] - changed_projection[i][0]*changed_projection[i][0]);
	    dE_2 += (2*diff_projection[i][changed_location[i][1][0]][changed_location[i][1][1]]*changed_projection[i][1] - changed_projection[i][1]*changed_projection[i][1]);
	  }
...
void print_diff_projection()
{
  ofstream fout3;

  fout3.open("diff_projection.txt");

  for(int i=0; i<N_p+1; i++)
    {
      for(int j=0; j<2*Lmax+1; j++)
	for(int k=0; k<2*Lmax+1; k++)
	  if(diff_projection[i][j][k]*diff_projection[i][j][k]!= 0)
	    {
	      fout3<<i<<"  "<<j<<"  "<<k<<"  "<<diff_projection[i][j][k]<<"  "<<diff_projection[i][j][k]*diff_projection[i][j][k]<<endl;
	    }
    }
